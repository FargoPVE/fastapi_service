...
@pytest.mark.parametrize(
    "user_id, email, hashed_password, exists",
    [
        (
            1,
            "alex@mail.ru",
            "$2b$12$cu8nV9BRwC3oX6EaaxVbIuirsUG6nudwEp4n6h/XMvDvEFkOBhlOq",
            True,
        ),
        (
            2,
...
async def test_find_all_users(
    user_id: int, email: str, hashed_password: str, exists: bool
):
...
async def test_find_one_or_none_users(
    user_id: int, email: str, hashed_password: str, exists: bool
):
